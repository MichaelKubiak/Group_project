...
function(input,output){
  output$plot1<-renderPlotly({
    cols<-c()
    for (type in sce$sc3_25_clusters){
      if (type %in% "1"){
        ...
        cols<-append(cols,input$Cluster_23)
      }else if (type %in% "24"){
        cols<-append(cols,input$Cluster_24)
      }else{
        cols<-append(cols,input$Cluster_25)
      }
    }
